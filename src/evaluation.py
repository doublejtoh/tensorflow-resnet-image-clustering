...
if __name__ == '__main__':
    ...
    parser.add_argument('--labels_pred1', help="labels prediction file name", required=True)
    parser.add_argument('--labels_pred2', help="labels prediction file name")
    args, _ = parser.parse_known_args()

    score1 = evaluation(os.path.join(TRAINING_DATA_DIR, LABELS_TRUE + ".txt"), os.path.join(TRAINING_DATA_DIR, args.labels_pred1))
    print("Score for %s: %s" % (args.labels_pred1, score1))

    if args.labels_pred2 is not None:
        score2 = evaluation(os.path.join(TRAINING_DATA_DIR, LABELS_TRUE + ".txt"), os.path.join(TRAINING_DATA_DIR, args.labels_pred2))
        print("Score for %s: %s" % (args.labels_pred2, score2))
